...
class T2IVAE(nn.Module):
    def __init__(self):
        ...
        self.img_size = 224 if self.config.DATASET == 'coco' else 32
    ...
    def forward(self, img, text_inputs, mask_img=False, mask_text=False):
        ...
        if self.args.sample_latent:
            ...
            combined_embedding = torch.randn_like(img_feat_means)
            combined_embedding_means = torch.zeros_like(img_feat_means)
            combined_embedding_logvars = torch.zeros_like(img_feat_means)
        else:
            ...
            if mask_img:
                ...
                combined_embedding_means, combined_embedding_logvars  = self.get_combined_embedding(torch.zeros_like(img_feat_means), text_feat_means)
            elif mask_text:
