...
class T2IVAE(nn.Module):
    def __init__(self):
        ...
        self.text_decoder_proj = nn.LazyLinear(512 * self.max_seq_len)
    ...
    def forward(self, img, text_inputs):
        ...
        img_feat_logvars = self.img_feat_logvars(flattened_img_feats) # TODO: lower logvars by subtracting a constant (e.g. 3-5ish)
        ...
        text_decoder_input = self.text_decoder_proj(sampled_text_latent).view(-1, self.max_seq_len, 512)
        ...
        text_decoder_out = self.t5.decoder(input_ids=text, encoder_hidden_states=text_decoder_input, encoder_attention_mask=text_inputs["attention_mask"].to(device))
